...
double median(double vett[]){
    double media=0;
     ...
     return media/RIPETIZIONI;
}

double sigma(double vett[],double media){
    double sigma=0;
        for(int j=0;j<RIPETIZIONI;j++){
                sigma= sigma + pow((vett[j]-media),2);
        }
        return sqrt(sigma/RIPETIZIONI);
}
    ...
    for(int i=0; i<100; i++){
            ...
            for(int j=0;j<RIPETIZIONI;j++){
                ...
                if(caso != 0) //caso medio o peggiore
                    ...
                    string_peggiore(stringaNuova,n);
